...
''')
    ...
    def loginUser(email, password, driver):
        try:
            ...
            driver.find_element_by_name('identifier').send_keys(email.get(), Keys.RETURN)
            sleep(2)

            driver.find_element_by_name('password').send_keys(password.get(), Keys.RETURN)
            sleep(5)
